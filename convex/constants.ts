export const VISA_PRICING = {
  ...
  germany: {
    ...
    visaTypes: [
      ...
      "Visa National — Regroupement familial (Familienzusammenführung)",
      "Visa National — Travail qualifié (Fachkräfteeinwanderung)",
      "Visa National — Chancenkarte (Carte d'opportunité)",
      "Visa National — Au-pair / Volontariat (FSJ/BFD)",
      "Visa National — Cours de langue / Formation",
      "Visa Schengen — Court séjour (Tourisme / Affaires)",
      "Visa Schengen — Visite familiale",
      "Visa Schengen — Traitement médical",
    ],
    requiredDocuments: [
      { key: "passport_scan", label: "Passeport valide (scan HD, 2 pages bio)", required: true },
      { key: "photo_id", label: "Photo d'identité biométrique 3,5×4,5 cm fond blanc", required: true },
      { key: "proof_of_funds", label: "Relevés bancaires (3 derniers mois) ou compte bloqué (Sperrkonto)", required: true },
      { key: "employment_letter", label: "Attestation de travail / RCCM / certificat de scolarité", required: true },
      { key: "travel_insurance", label: "Assurance maladie voyage (min. 30 000 €)", required: true },
      { key: "motivation_letter", label: "Lettre de motivation / CV (Visa National)", required: false },
      { key: "university_admission", label: "Lettre d'admission universitaire (études)", required: false },
      { key: "work_contract", label: "Contrat de travail / promesse d'embauche (travail)", required: false },
      { key: "language_certificate", label: "Certificat de langue (A1/B1 selon catégorie)", required: false },
      { key: "blocked_account", label: "Attestation compte bloqué (Sperrkonto ~11.208 €/an)", required: false },
      { key: "invitation_letter", label: "Lettre d'invitation / attestation d'hébergement", required: false },
    ],
    embassyAddress: "Ambassade d'Allemagne — 82, Avenue Roi Baudouin, Kinshasa-Gombe",
    portalUrl: "https://service2.diplo.de/rktermin/extern/choose_realmList.do?locationCode=kins&request_locale=en",
    portalName: "RK-Termin — Auswärtiges Amt (service2.diplo.de)",
    portalDashboardUrl: "https://service2.diplo.de/rktermin/extern/choose_realmList.do?locationCode=kins&request_locale=en",
    portalAppointmentUrl: "https://service2.diplo.de/rktermin/extern/choose_realmList.do?locationCode=kins&request_locale=en",
    portalScheduleUrl: "https://service2.diplo.de/rktermin/extern/appointment_showMonth.do",
    processingType: "appointment",
    successCopy: {
      triggerLabel: "Créneau RK-Termin capturé",
      clientCtaTitle: "Créneau Allemagne trouvé !",
      clientCtaBody: "Joventy a verrouillé un rendez-vous à l'ambassade d'Allemagne via le portail RK-Termin. Réglez la prime de succès pour recevoir votre convocation officielle avec la date, l'heure et les instructions de dépôt.",
      completedNote: "Votre convocation RK-Termin est disponible ci-dessous — présentez l'email de confirmation imprimé le jour du rendez-vous avec vos documents originaux.",
    },
    notes: "Portail RK-Termin (version 1.3.0.4) — système Java/Struts2 avec captcha image simple. Deux modes : (A) calendrier avec sélection de date/heure, (B) liste d'attente avec attribution automatique ~4 semaines avant le RDV. Kinshasa — locationCode 'kins', realmId 731 (Visa National congolais), 1276 (Schengen), 1505 (Visa National tiers). Frais consulaires : 75 € (Visa National), 80 € (Visa Schengen adulte). Délai de traitement : 6-12 semaines (National), 15 jours (Schengen). Captcha résolvable via 2Captcha/CapSolver (~$0.002/résolution). Pas de Cloudflare, pas de WAF détecté.",
  },
  morocco: {
    label: "Maroc",
    flag: "🇲🇦",
    ...
    visaTypes: [
      ...
      "Visa Consulaire — Tourisme / Affaires (sans rendez-vous)",
      "Transit Ebola 21 jours",
    ],
    requiredDocuments: [
      { key: "passport_scan", label: "Passeport valide (scan HD, 2 pages bio)", required: true },
      { key: "photo_id", label: "Photo d'identité fond blanc", required: true },
      { key: "hotel_booking", label: "Réservation d'hôtel ou Airbnb au Maroc", required: true },
      { key: "flight_booking", label: "Billet d'avion aller-retour", required: true },
      { key: "proof_of_funds", label: "Relevés bancaires (3 derniers mois)", required: true },
      { key: "employment_letter", label: "Attestation de travail / RCCM", required: false },
    ],
    embassyAddress: "Ambassade du Maroc — Avenue de la Gombe, Kinshasa-Gombe",
    portalUrl: "https://evisa.marocmfa.ma",
    portalName: "E-Visa Maroc — portail officiel MFA",
    portalDashboardUrl: "https://evisa.marocmfa.ma",
    portalAppointmentUrl: "https://evisa.marocmfa.ma",
    portalScheduleUrl: "https://evisa.marocmfa.ma",
    processingType: "hybrid",
    successCopy: {
      triggerLabel: "Visa obtenu",
      clientCtaTitle: "Votre visa Maroc est prêt !",
  ...
  egypt: {
    ...
    visaTypes: [
      ...
      "Visa Consulaire — Tourisme (sans rendez-vous)",
      "Transit Ebola 21 jours",
    ],
    requiredDocuments: [
      { key: "passport_scan", label: "Passeport valide (scan HD, 2 pages bio)", required: true },
      { key: "photo_id", label: "Photo d'identité fond blanc", required: true },
      { key: "hotel_booking", label: "Réservation d'hôtel en Égypte", required: true },
      { key: "flight_booking", label: "Billet d'avion aller-retour", required: true },
      { key: "proof_of_funds", label: "Relevés bancaires récents", required: true },
    ],
    embassyAddress: "Ambassade d'Égypte — Avenue de la Gombe, Kinshasa-Gombe",
    portalUrl: "https://visa2.egypt.gov.eg",
    portalName: "E-Visa Égypte — portail officiel gouvernemental",
    portalDashboardUrl: "https://visa2.egypt.gov.eg",
    portalAppointmentUrl: "https://visa2.egypt.gov.eg",
    portalScheduleUrl: "https://visa2.egypt.gov.eg",
    processingType: "hybrid",
    successCopy: {
      triggerLabel: "Visa obtenu",
      clientCtaTitle: "Votre visa Égypte est prêt !",
...
// ─── Tarification créneaux (système unifié, prix promo) ────────────────────
export const CRENEAU_PROMO_PRICING = {
  normalPrice: 350,       // Prix affiché — 350 $ payés après obtention
  promoPrice: 350,        // Prix actuel (= normalPrice, pas de promo distincte)
  depositAmount: 0,       // Aucun acompte
  successAmount: 350,     // Solde dû à l'obtention du créneau
...
export const SERVICE_PACKAGES = {
  ...
  slot_only: {
    ...
    description: "Vos formulaires sont remplis et vos pièces prêtes ? Joventy se concentre uniquement sur l'obtention d'un créneau de rendez-vous consulaire. À choisir si vous voulez seulement le rendez-vous, pas la préparation du dossier complet.",
    hasSuccessFee: true,
