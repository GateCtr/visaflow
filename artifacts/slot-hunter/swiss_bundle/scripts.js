 ...
 *
 * @param {string}
 *            plaintext - String to be encrypted (multi-byte safe).
 * @param {string}
 *            password - Password to be used for encryption (1st 16 chars).
...
function d(ciphertext, password) {
 ...
 * @param {string}
 *            password - Password to be used for decryption (1st 16 chars).
...
Tea.strToLongs = function(s) {
	// note chars must be within ISO-8859-1 (Unicode code-point <= U+00FF) to
	// fit 4/long
	var l = new Array(Math.ceil(s.length / 4));
	for (var i = 0; i < l.length; i++) {
		// note little-endian encoding - endianness is irrelevant as long as it
		// matches longsToStr()
...
Tea.longsToStr = function(l) {
	...
	return a.join(''); // use Array.join() for better performance than repeated
						// string appends
};
 ...
 * nodejs.org/api/buffer.html note: if btoa()/atob() are not available (eg
 * IE9-), try github.com/davidchambers/Base64.js
 */
...
if (typeof module != 'undefined' && module.exports)
	module.exports = Tea; // CommonJS export
