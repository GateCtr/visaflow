/**
 * test-saopola-multidossier.ts — Test live multi-dossier en parallèle (Saopola)
 *
 ...
 *
 * Usage : cd artifacts/slot-hunter && npx tsx src/test-saopola-multidossier.ts
 *         SAOPOLA_N=3 npx tsx src/test-saopola-multidossier.ts  (N dossiers, défaut=2)
 */
...
const N_DOSSIERS = process.env.SAOPOLA_N ? parseInt(process.env.SAOPOLA_N, 10) : 2;
...
const FAKE_DOSSIERS: SpainBookingConfig[] = [
  {
    login: "00000001",
    password: "FakePass_Dossier_A",
    applicantName: "JEAN DUPONT (dossier A)",
    applicantEmail: "jean.dupont.fake@gmail.com",
    visaType: "visa touristique",
  },
  {
    login: "00000002",
    password: "FakePass_Dossier_B",
    applicantName: "MARIE KONE (dossier B)",
    applicantEmail: "marie.kone.fake@gmail.com",
    visaType: "visa touristique",
  },
  {
    login: "00000003",
    password: "FakePass_Dossier_C",
    applicantName: "PAUL MBEKI (dossier C)",
    applicantEmail: "paul.mbeki.fake@gmail.com",
    visaType: "visa touristique",
  },
  {
    login: "00000004",
    password: "FakePass_Dossier_D",
    applicantName: "SARAH NZOLA (dossier D)",
    applicantEmail: "sarah.nzola.fake@gmail.com",
    visaType: "visa touristique",
  },
].slice(0, N_DOSSIERS);
...
async function main() {
  sep(`TEST MULTI-DOSSIER — ${N_DOSSIERS} dossier(s) en parallèle (Saopola)`);
  ...
  if (scanResult.status === "not_found") {
    console.log("\n  ℹ️  Aucun créneau disponible — portail répond correctement. Réessaie quand des créneaux sont visibles.");
    process.exit(0);
  }
  ...
  const results = await Promise.all(
    FAKE_DOSSIERS.map(async (dossier, i) => {
      const t0 = Date.now();
  ...
  // (pas de crash ni de confusion de PHPSESSID qui aurait retourné 0B en masse)
  ...
  const overhead = ((parallelDur - serialExpected) / serialExpected * 100).toFixed(0);
