...
const START_URL = getArg('url', `${BASE_URL}/en-ma/niv/users/sign_in`);
...
function setupCapture(page: Page): void {
  page.on('request', req => {
    ...
    const url = req.url();
    if (url.includes('usvisa-info.com') && (
...
function saveAll(): void {
  ...
  const summary = {
    ...
    endpoints: [...new Set(requests
      .filter((r: any) => r.url.includes('usvisa-info.com'))
...
async function main(): Promise<void> {
  ...
  console.log(`🖥️  Xvfb démarré sur ${display}`);
