...
export default function SchengenAlerts() {
  ...
  const orders = useQuery(
    api.schengenAlert.listOrders,
    statusFilter === "all" ? {} : { status: statusFilter as "pending" | "confirmed" | "rejected" }
  );
  const stats = useQuery(api.schengenAlert.getStats, {});

  const confirmOrder = useMutation(api.schengenAlert.confirmOrder);
  const rejectOrder = useMutation(api.schengenAlert.rejectOrder);

  const handleConfirm = async (orderId: Id<"schengenAlertOrders">) => {
  ...
  const handleReject = async (orderId: Id<"schengenAlertOrders">) => {
    try {
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-black text-primary">Alertes Schengen 🇪🇺</h1>
            <p className="text-muted-foreground text-sm">Groupe WhatsApp · 10 USD / accès à vie</p>
          </div>
        </div>
        <a href="/alerte-schengen" target="_blank" rel="noreferrer" className="flex items-center gap-1.5 text-xs text-primary hover:underline font-medium">
                    ...
                    <div className="mt-4 pt-4 border-t border-border flex flex-col sm:flex-row gap-3">
                      {rejectingId === order._id ? (
                        <div className="flex-1 flex flex-col sm:flex-row gap-2">
                          ...
                          />
                          <Button variant="destructive" size="sm" onClick={() => handleReject(order._id as Id<"schengenAlertOrders">)}>
                            Confirmer le rejet
                          </Button>
                        ...
                        <>
                          <Button
                            onClick={() => handleConfirm(order._id as Id<"schengenAlertOrders">)}
