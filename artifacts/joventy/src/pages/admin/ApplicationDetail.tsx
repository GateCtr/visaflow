...
export default function AdminApplicationDetail() {
  ...
  const [slotScreenshotFile, setSlotScreenshotFile] = useState<File | null>(null);
  const [slotSaving, setSlotSaving] = useState(false);
  const slotScreenshotRef = useRef<HTMLInputElement>(null);
  ...
  const handleMarkSlot = async () => {
    ...
    try {
      ...
      if (slotScreenshotFile) {
        const uploadUrl = await generateUploadUrl();
        const res = await fetch(uploadUrl, { method: "POST", headers: { "Content-Type": slotScreenshotFile.type }, body: slotScreenshotFile });
        if (!res.ok) throw new Error("Échec de l'upload du screenshot");
      ...
      setSlotDate(""); setSlotTime(""); setSlotCode(""); setSlotScreenshotFile(null);
      if (slotScreenshotRef.current) slotScreenshotRef.current.value = "";
    } catch (err: unknown) {
              ...
              {(isSlotFound || isCompleted) && app.appointmentDetails ? (
                  ...
                  {confirmationLetterUrl && (
                    <div className="mt-3 pt-3 border-t border-green-200">
                      <p className="text-xs font-semibold text-green-800 mb-2 flex items-center gap-1.5">
                        <FileText className="w-3.5 h-3.5" /> Page de confirmation enregistrée
                      </p>
                      <a
                        href={confirmationLetterUrl}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-2 text-xs text-primary font-medium bg-white border border-green-200 rounded-lg px-3 py-2 hover:bg-green-50 transition-colors"
                      >
                        <Eye className="w-3.5 h-3.5" /> Voir la lettre de confirmation
                      </a>
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="sm:col-span-2 space-y-1.5">
                      <label className="text-xs font-medium text-muted-foreground uppercase">Page de confirmation (PDF/Image — optionnel)</label>
                      <div className="flex items-center gap-3">
                        <input
                          ref={slotScreenshotRef}
                          type="file"
                          accept="image/*,application/pdf"
                          className="hidden"
                          onChange={(e) => {
                            const f = e.target.files?.[0];
                            if (f) setSlotScreenshotFile(f);
                          }}
                        />
                        <Button
                          type="button"
                          ...
                          onClick={() => slotScreenshotRef.current?.click()}
                        >
                          <Upload className="w-3.5 h-3.5" />
                          {slotScreenshotFile ? "Changer le fichier" : "Choisir un fichier"}
                        </Button>
                        {slotScreenshotFile && (
                          <span className="text-xs text-green-700 font-medium truncate max-w-[200px]">{slotScreenshotFile.name}</span>
                        )}
                      </div>
                      <p className="text-[11px] text-muted-foreground mt-1">Ce document sera la lettre de convocation envoyée au client après paiement de la prime.</p>
                    </div>
                  </div>
                  <Button
