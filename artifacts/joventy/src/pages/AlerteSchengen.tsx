...
const OBJECTIONS = [
  ...
  },
  {
    ...
    a: "Un café à Bruxelles coûte 4 EUR. 10 USD vous donne un accès à vie à des alertes précises sur visaonweb.be, une méthode testée, et une communauté active. Comparé aux 350 USD du service créneau Joventy (payé après résultat), c'est la meilleure valeur disponible pour ceux qui veulent le faire eux-mêmes.",
  },
  {
...
export default function AlerteSchengen() {
  const { toast } = useToast();
  const generateUploadUrl = useMutation(api.schengenAlert.generateUploadUrl);
  const submitOrder = useMutation(api.schengenAlert.submitOrder);
        ...
        <div className="max-w-3xl mx-auto">
          <div className="bg-primary rounded-3xl p-8 sm:p-12 text-white text-center">
            ...
            <p className="text-white/70 text-base mb-8 max-w-xl mx-auto leading-relaxed">
              D'autres paient 350 USD pour qu'on gère leur rendez-vous (après résultat). Vous préférez le faire vous-même sur visaonweb.be — c'est tout à fait possible. Ce groupe vous donne les outils pour y arriver. 10 USD, accès à vie, aucun abonnement.
            </p>
            ...
            <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-border shadow-lg overflow-hidden">
              ...
              <div className="p-6 border-b border-border">
                ...
                {file ? (
                  ...
                  <button type="button" onClick={() => fileRef.current?.click()}
                    className="w-full border-2 border-dashed border-border rounded-xl p-8 flex flex-col items-center gap-3 hover:border-primary/40 hover:bg-primary/2 transition-all group"
                  >
                    <Upload className="w-8 h-8 text-muted-foreground group-hover:text-primary transition-colors" />
                    <div className="text-center">
                      <p className="font-semibold text-sm text-primary">Cliquez pour uploader</p>
                      <p className="text-xs text-muted-foreground mt-1">Image ou PDF · Max 10 MB</p>
                    </div>
                  </button>
                )}
              </div>
              ...
              <div className="p-6">
                <Button
                  ...
                  className="w-full bg-secondary hover:bg-secondary/90 text-white font-black text-base py-4 rounded-xl shadow-lg shadow-secondary/20 gap-2 h-auto"
                >
                  {isSubmitting ? (
                    <>
                    ...
                    <>
                      ...
                      Envoyer ma demande d'accès · 10 USD
                    </>
                  )}
                </Button>
                <p className="text-xs text-center text-muted-foreground mt-3 leading-relaxed">
                  Votre demande sera vérifiée par l'admin. Vous recevrez le lien du groupe WhatsApp par email dans les 24h.
                </p>
              </div>
            </form>
          )}
