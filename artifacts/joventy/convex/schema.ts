...
const hunterConfig = v.object({
  ...
  // Dossiers exclus du scan (jamais scannés même s'ils sont dans le pool)
...
export default defineSchema({
  ...
  spainWatcherScans: defineTable({
    ...
    screenshotStorageId: v.optional(v.string()),
    errorMessage: v.optional(v.string()),
    // Dossier associé (worker Spain multi-dossier)
    applicationId: v.optional(v.string()),  // ID Convex du dossier (string pour compatibilité HTTP)
    dossierName: v.optional(v.string()),    // Nom du demandeur (pour affichage sans join)
